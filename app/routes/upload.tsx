...
const upload = () => {
  ...
  const handleAnalyse = async ({ companyName, jobTitle, jobDescription, file } : {companyName: string, jobTitle: string, jobDescription: string, file: File}) => {
    ...
    console.log('PDF to Image result:', imageFile); // Add this line for debugging
    if(!imageFile.file) return setStatusText('Error failed to convert PDF to image');

    setStatusText('Uploading the image....');
    const uploadedImage = await fs.upload([imageFile.file]);
    if(!uploadedImage) return setStatusText('Error: Failedto upload image');
        ...
        <div className="page-heading py-4">
          ...
          {!isProcessing && (
            ...
            >
              ...
              <button className="primary-button" typeof="submit">
                Analyse resume
              </button>
            </form>
